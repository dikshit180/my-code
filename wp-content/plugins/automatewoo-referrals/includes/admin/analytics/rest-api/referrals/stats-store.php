...
class Stats_Store extends Generic_Stats_Store {
	...
	/**
	 * Backward compatibility layer for WC <9.3 & AW <= 6.0.32.
	 *
	 * Compatibility-code "WC < 9.3, AW <= 6.0.32" -- function signature and logic flow changed in WC 9.3
	 *
	 * @see get_data
	 ...
	 */
	public function get_noncached_data( $query_args, $params = null, &$data = null, $expected_interval_count = null ) {
		if ( version_compare( AW()->version, '6.0.32', '<=' ) ) {
			return $this->get_noncached_stats_data( $query_args, $params, $data, $expected_interval_count );
		} else {
			return parent::get_noncached_data( $query_args );
		}
	}
