...
class Hooks {
	...
	function __construct() {
		...
		if ( AW_Referrals()->options()->get_reward_event() === 'purchase' ) {
			//When the checkout pages use blocks the woocommerce_checkout_order_processed hook is not being called, therefore we use woocommerce_store_api_checkout_order_processed
			add_action( 'woocommerce_store_api_checkout_order_processed', [ 'AutomateWoo\Referrals\Referral_Manager', 'check_order_for_referral' ], 110 ); // must run after subscriptions are created
			...
			add_action( 'woocommerce_payment_complete', [ 'AutomateWoo\Referrals\Referral_Manager', 'clear_link_cookie' ], 110, 0 );
