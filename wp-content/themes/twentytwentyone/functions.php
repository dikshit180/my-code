...
function rv_movie_cpt() {
	...
	$labels = array(
		'name'               => _x( 'Hotel', 'post type general name', 'engwp' ),
		'singular_name'      => _x( 'Hotel', 'post type singular name', 'engwp' ),
		'menu_name'          => _x( 'Hotels', 'admin menu', 'engwp' ),
		'name_admin_bar'     => _x( 'Hotel', 'add new on admin bar', 'engwp' ),
		'add_new'            => _x( 'Add New', 'Hotel', 'engwp' ),
		'add_new_item'       => __( 'Add New Hotel', 'engwp' ),
		'new_item'           => __( 'New Hotel', 'engwp' ),
		'edit_item'          => __( 'Edit Hotel', 'engwp' ),
		'view_item'          => __( 'View Hotel', 'engwp' ),
		'all_items'          => __( 'All Hotels', 'engwp' ),
		'search_items'       => __( 'Search Hotels', 'engwp' ),
		'parent_item_colon'  => __( 'Parent Hotel:', 'engwp' ),
		'not_found'          => __( 'No Hotel found.', 'engwp' ),
		'not_found_in_trash' => __( 'No Hotel found in Trash.', 'engwp' )
	);

	$args = array(
		'description'	      => __( 'Hotel', 'engwp' ),
		'labels'	      => $labels,
...
function custom_type_taxonomy() {
	...
	register_taxonomy( 'hotel_location', 'hotel',
		array(
			'labels' => array(
				'name'          => _x( 'Hotel Categoary', 'taxonomy general name', 'text_domain' ),
				'add_new_item'  => __( 'Add New Hotel Categoary', 'text_domain' ),
				'new_item_name' => __( 'New Hotel Location', 'text_domain' ),
			),
			'exclude_from_search' => true,
			'has_archive'         => true,
			'hierarchical'        => true,
			'rewrite'             => array( 'slug' => 'hotel-location', 'with_front' => false ),
			'show_ui'             => true,
			'show_tagcloud'       => false,
		)
	);
...
add_shortcode('hotel-form', 'hotel_form_code');
 wp_enqueue_script(
        'my-custom-script', 
...
function hotel_form_code() {
	ob_start(); 
	?>
	<form id="form-hotel" action="" enctype= multipart/form-data>
		<label for="hotel-loc">Choose a Location</label>
		<select name="location" id="hotel-loc">
			<?php
			$args = [
		...
		<input type="hidden" name="action" value="hotel"><br>
		<input type="text" name="hotel-name" placeholder="Enter Hotel Name" required><br>
		<input type="hidden" name="security" value="<?php echo wp_create_nonce('hotel_nonce'); ?>">
		<textarea name="hot-con" placeholder="Enter Content" required></textarea><br>
		<input type="file" name="hotel-image"><br>
		<input type="submit" name="submit-hotel" >
	</form>
	<div class="submit-hotel"></div>
	<?php
	return ob_get_clean(); 
}
add_action( 'wp_ajax_nopriv_hotel', 'hotel_insert' );
add_action( 'wp_ajax_hotel', 'hotel_insert' );

function hotel_insert() {
	check_ajax_referer('hotel_nonce', 'security');
   $name=sanitize_text_field( $_POST['hotel-name'] );
   $content=sanitize_text_field( $_POST['hot-con'] );
   $cat_id=intval($_POST['location']);
   $post_id = wp_insert_post( array(
                    'post_status' => 'publish',
                    'post_type' => 'hotel',
                    'post_title' =>$name ,
    ...
    if ($cat_id) {
        wp_set_post_terms($post_id, array($cat_id), 'hotel_location');
    }
	if(!empty ($_FILES['hotel-image']['name']))
	{
		
require_once( ABSPATH . 'wp-admin/includes/image.php' ); // Required for image processing functions
require_once( ABSPATH . 'wp-admin/includes/file.php' ); // Required for file handling functions
require_once( ABSPATH . 'wp-admin/includes/media.php' ); // Required for media handling functions
$attach_id = media_handle_upload( 'hotel-image', $post_id );
if(!is_wp_error($attach_id))
{
	set_post_thumbnail($post_id, $attach_id);
	  ...
	  wp_send_json_success('Hotel Post Submitted Sucessfully');
}

add_action('admin_menu', 'my_hotel_posts_menu_page');
function my_hotel_posts_menu_page() {
    add_menu_page(
        'Hotel Posts',
        'Hotel Posts',
        'manage_options',
        'hotel-posts-list',
        'render_hotel_posts_table_page',
        'dashicons-building',
        6
    );
}
...
class Hotel_Posts_List_Table extends WP_List_Table {
    ...
    public function get_columns() {
        return [
            ...
            'title'    => 'Title',
            'location' => 'Location', // taxonomy column
            'author'   => 'Author',
            'date'     => 'Date'
        ];
    }
    ...
    public function prepare_items() {
        ...
        foreach ($query->posts as $post) {
            // Get taxonomy terms
            $terms = get_the_terms($post->ID, 'hotel_location');
            $term_names = [];
    ...
    public function column_title($item) {
        ...
        return '<a href="' . esc_url($link) . '">' . esc_html($item['title']) . '</a>';
    }
...
function render_hotel_posts_table_page() {
    echo '<div class="wrap"><h1>Hotel Posts</h1>';
    $table = new Hotel_Posts_List_Table();
    $table->prepare_items();
